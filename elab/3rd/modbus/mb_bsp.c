                    ...
                    uint8_t stops)
{
    ...
    elab_serial_config_t uart_config =
    {
        baud,
        bits,
        ...
        PLATFORM_SERIAL_FLOWCONTROL_NONE,
    };
    ...
    {
        ret = elab_io_close(mb_dev_data[pch->PortNbr].serial);
        if (ELAB_OK != ret)
        {
            elog_error("Modbus serial port closing fails. Error ID: %ld.",
                        (int32_t)ret);
            goto exit;
        }
    }
    ...
    ret = elab_io_control(mb_dev_data[pch->PortNbr].serial,
                                    ELAB_IO_CTRL_UART_CONFIGURE,
                                    (void *)&uart_config);
    if (ELAB_OK != ret)
    {
        elog_error("Modbus serial port config fails. Error ID: %ld.",
                     (int32_t)ret);
        goto exit;
    }

    /* Open the serial device. */
    ret = elab_io_open(mb_dev_data[pch->PortNbr].serial);
    if (ELAB_OK != ret)
    {
        elog_error("Modbus serial port opening fails. Error ID: %ld.",
                     (int32_t)ret);
        goto exit;
    }
...
static uint32_t mb_port_write(elab_mb_channel_t *pch, const void *pbuf, uint32_t size)
{
    ...
    uint8_t nbr = pch->PortNbr;
    if (MB_DEV_TYPE_UART == mb_dev_info[nbr].type ||
        MB_DEV_TYPE_RS485 == mb_dev_info[nbr].type)
    {
        elab_device_t *serial = mb_dev_data[nbr].serial;
        ret = elab_io_write(serial, 0, pbuf, size);
    }
    else
    {
...
static uint32_t mb_port_read(elab_mb_channel_t *pch, void *pbuf, uint32_t size)
{
    ...
    {
        elab_device_t *serial = mb_dev_data[nbr].serial;
        ret = elab_io_read(serial, 0, pbuf, size);
    }
    else
    {
        rs485_t *rs485 = (rs485_t *)mb_dev_data[nbr].dev_handle;
